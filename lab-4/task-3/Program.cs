...
namespace LightHTML
{
    public class Program
    {
        public static void Main()
        {
            ...
            Console.WriteLine("\n=== Демонстрація роботи EventListener ===");

            var courseList = FindElementByClass(container, "course-list") as LightElementNode;
            if (courseList != null && courseList.ChildCount > 0)
            {
                var firstCourseItem = courseList.GetChildren()[0] as LightElementNode;
                if (firstCourseItem != null)
                {
                    firstCourseItem.AddEventListener("click", eventType =>
                    {
                        Console.WriteLine($"Клік був натиснутий! Подія: {eventType}");
                    });

                    Console.WriteLine("Симулюємо клік на перший курс:");
                    firstCourseItem.TriggerEvent("click");
                }
            }

            container.AddEventListener("mouseover", eventType =>
            {
                Console.WriteLine($"Курсор наведено на контейнер! Подія: {eventType}");
            });

            Console.WriteLine("\nСимулюємо подію mouseover на контейнері:");
            container.TriggerEvent("mouseover");
        }

        private static LightNode FindElementByClass(LightElementNode root, string cssClass)
        {
            if (root.CssClasses.Contains(cssClass))
                return root;
            ...
            foreach (var child in root.GetChildren())
            {
                ...
                {
                    var result = FindElementByClass(elementNode, cssClass);
                    if (result != null)
                        return result;
                }
            }
    ...
    {
        public static List<LightNode> GetChildren(this LightElementNode node)
        {
            var field = typeof(LightElementNode).GetField("children",
                  System.Reflection.BindingFlags.NonPublic |
                  System.Reflection.BindingFlags.Instance);

            return field?.GetValue(node) as List<LightNode> ?? new List<LightNode>();
        }
    }
}