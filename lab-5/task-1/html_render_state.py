...
class MinifiedRenderState(RenderState):
    def render(self, element: HTMLElement, context: RenderContext) -> str:
        self.context = context
        return self._render_element(element)
        
    def _render_element(self, element: HTMLElement) -> str:
        attributes = self._format_attributes(element.attributes)
        
        if not element.children and element.tag_name in self.context.self_closing_tags:
            return f"<{element.tag_name}{attributes}/>"
            
        opening = f"<{element.tag_name}{attributes}>"
        if not element.children:
            return f"{opening}</{element.tag_name}>"
            
        content = ''.join(self._render_element(child) for child in element.children)
        
        return f"{opening}{content}</{element.tag_name}>"
